...
public class ChunkManager : MonoBehaviour
{
    ...
    public void Init(int width, int height)
    {
        ...
        {
            _chunkContainer = transform.GetChild(0).gameObject;
        }    
    ...
    private void LoadChunkFromChunkData(ChunkData chunkData)
    {
        ...
        Chunk chunk = _chunks[chunkData.ChunkCoordX, chunkData.ChunkCoordY];
        if (chunk == null)
        {
        ...
        GameObject chunkObj = _chunkObjs[chunkData.ChunkCoordX, chunkData.ChunkCoordY];
        if (chunkObj == null)
        {
        ...
        chunkObj.name = "Chunk (" + chunkData.ChunkCoordX + ", " + chunkData.ChunkCoordY + ")";
        chunkObj.transform.position = chunk.WorldPosition;

        chunkObj.GetComponent<MeshFilter>().sharedMesh = chunk.Mesh;
        chunkObj.GetComponent<MeshCollider>().sharedMesh = chunk.Mesh;

        _chunkObjs[chunkData.ChunkCoordX, chunkData.ChunkCoordY] = chunkObj;
    }
    ...
    public void GetChunkObjectsFromScene()
    {
        ...
        MeshFilter[] chunksInScene = _chunkContainer.GetComponentsInChildren<MeshFilter>();
    ...
    {
        GameObject chunkGameObject = Instantiate(_chunkObject, Vector3.zero, Quaternion.identity, _chunkContainer.transform);
